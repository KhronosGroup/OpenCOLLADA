...
namespace COLLADA
{
			...
			, mHasColors(false)
    {
        ...
        {
            COLLADA_EXCEPT(COLLADA::Exception::ERR_INTERNAL_ERROR, "", "TriangleIterator::TriangleIterator");
        }
    }
	//-----------------------------------------------------------------------
...

    {
        for ( daeUInt i = 0; i < array.getCount(); i++ )
        {
            ...
            {
                offset = (daeSafeCast<domInputLocalOffset>(input))->getOffset();
    ...
    void TriangleIterator::getVertex( domFloat *position, domFloat *normal, domFloat *color, domFloat *tex, int &_order )
    {
        ...
        for ( size_t i = 0; i < numElementsToRead; i++ )
        {
            ...
            domUint index = list.get( indexInPList );
