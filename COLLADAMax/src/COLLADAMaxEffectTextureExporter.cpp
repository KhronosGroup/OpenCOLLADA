...
namespace COLLADAMax
{
        ...
        const String& targetPath )
    {
        ...
        colladaTexture->addExtraTechniqueParameter ( PROFILE_MAX, MAX_TEXTURE_BLENDMODE_PARAMETER, blendModeString );
...
//        colladaImage->addExtraTechniqueParameter ( PROFILE_MAYA, PARAMETER_MAYA_ID, mayaImageId );
    ...
    {
        // Target file
        String targetFile = mDocumentExporter->getOutputFileUri().getPath();
...
*/
        ...
        COLLADASW::URI &fullFileNameURI )
    {
        ...
        if ( mDocumentExporter->getOptions().getExportRelativePaths() )
        {
            ...
            if ( mDocumentExporter->getOptions().getCopyImages() )
            {
                ...
                String targetColladaFile = mDocumentExporter->getOutputFileUri().getPath();
                ...
                if ( !success ) 
                {
                    ...
                    GetCOREInterface()->Log()->LogEntry( SYSLOG_ERROR, DISPLAY_DIALOG, _M( "Error" ),_M( "%s\n" ), message.c_str() );
    ...
    const String EffectTextureExporter::findColladaImageId ( const String& maxImageId )
    {
        const StringToStringMap::const_iterator it = mMaxIdColladaImageId.find ( maxImageId );
        if ( it != mMaxIdColladaImageId.end () )
        {
            return it->second;
        }
        return "";
