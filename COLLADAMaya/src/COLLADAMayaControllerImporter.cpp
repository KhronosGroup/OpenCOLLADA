...
namespace COLLADAMaya
{
    ...
    */
        ...
        COLLADAFW::MorphController* morphController )
    {
        ...
        if ( COLLADABU::Utils::equals ( blendShapeName, EMPTY_STRING ) ) return false;
        ...
        BlendShapeData& blendShapeData )
    {
        ...
        if ( COLLADABU::Utils::equals ( blendShapeName, EMPTY_STRING ) ) blendShapeName = BLEND_SHAPE_NAME;
        ...
        if ( !COLLADABU::Utils::equals ( originalMayaId, EMPTY_STRING ) ) blendShapeName = originalMayaId;
        ...
        {
            std::cerr << "The number of morph targets differs from the number of morph weights!" << endl;
            return EMPTY_STRING;
        }
...
//         MayaDM::endSetAttr ( file );
        ...
        const COLLADAFW::SkinControllerData* skinControllerData )
    {
        ...
        for ( size_t i=0; i<skinControllers.size (); ++i )
        {
            ...
            if ( COLLADABU::Utils::equals ( skinClusterName, EMPTY_STRING ) ) return false;
            ...
            if ( !bindShapeMatrix.isIdentiy () )
            {
                ...
                if ( transformIdVec != 0 )
                {
                    for ( size_t i=0; i<transformIdVec->size (); ++i )
                    {
                        ...
                        // TODO Maya bind shape bug on unit conversion?
                        ...
                        MayaDM::setAttr ( file, "t", "lock", true, EMPTY_STRING );
                        ...
                        MayaDM::setAttr ( file, "r", "lock", true, EMPTY_STRING );
                        MayaDM::setAttr ( file, "ro", "lock", true, EMPTY_STRING );
                        ...
                        MayaDM::setAttr ( file, "s", "lock", true, EMPTY_STRING );
...
//             }
        ...
        SkinClusterData& skinClusterData )
    {
        ...
        if ( COLLADABU::Utils::equals ( skinClusterName, EMPTY_STRING ) ) skinClusterName = SKIN_CLUSTER_NAME;
        ...
        if ( !COLLADABU::Utils::equals ( originalMayaId, EMPTY_STRING ) ) skinClusterName = originalMayaId;
...
//             gm[3][2] = toLinearUnit ( gm[3][2] );
    ...
    // --------------------------------------------
    const std::vector<COLLADAFW::UniqueId>* ControllerImporter::findMorphTargets ( 
        const COLLADAFW::UniqueId& controllerId )
    {
        std::map<COLLADAFW::UniqueId, std::vector<COLLADAFW::UniqueId> >::const_iterator it = mMorphTargetIds.find ( controllerId );
        ...
        const COLLADAFW::UniqueId& controllerId )
    {
        ...
        if ( skinControllers == 0 ) 
        {
            ...
            std::cerr << "The morph controller is not used!" << endl;
            return transformIds;
        }
        for ( size_t i=0; i<skinControllers->size (); ++i )
        {
            ...
            if ( transformIds == 0 )
            {
                ...
                std::cerr << "The skin controller is not used!" << endl;
            }
            else
            {
                return transformIds;
    ...
    void ControllerImporter::connectGeometries ()
    {
        ...
        while ( it != mGeometryFilterDataMap.end () )
        {
            ...
            if ( controller == 0 ) 
            {
                std::cerr << "No controller element found!" << endl;
                ++it; continue;
            }
            ...
            {
                std::cerr << "No meshControllerData!" << endl;
                ++it; continue;
            }
    ...
    void ControllerImporter::connectMorphControllerTargets ()
    {
        ...
        while ( it != mGeometryFilterDataMap.end () )
        {
            ...
            if ( geometryFilterData->getGeometryFilterType () == GeometryFilterData::GEOMETRY_FILTER_TYPE_BLEND_SHAPE )
            {
                ...
                const std::vector<COLLADAFW::UniqueId>* morphTargets = findMorphTargets ( controllerId );
                if ( morphTargets != 0 )
                {
                    for ( size_t targetIndex=0; targetIndex<morphTargets->size (); ++targetIndex )
                    {
                        ...
                        if ( mesh == 0 )
                        {
                            std::cerr << "No mesh object for the current morph target!" << endl;
                            continue;
                        }
