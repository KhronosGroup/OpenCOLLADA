...
namespace COLLADAMaya
{
    ...
    bool MaterialImporter::importMaterial ( const COLLADAFW::Material* material )
    {
        ...
        if ( COLLADABU::Utils::equals ( materialName, EMPTY_STRING ) ) materialName = MATERIAL_NAME;
        ...
        if ( !COLLADABU::Utils::equals ( originalMayaId, EMPTY_STRING ) ) materialName = originalMayaId;
         ...
         String shadingEngineName /*= EMPTY_STRING*/ )
     {
         ...
         else
         {
             ...
             if ( COLLADABU::Utils::equals ( shadingEngineName, EMPTY_STRING ) ) shadingEngineName = SHADING_ENGINE_NAME;
    ...
    void MaterialImporter::createUVChoosers ()
    {
        ...
        while ( it != mBindingTexCoordInputSetsMap.end () )
        {
            ...
            for ( size_t i=0; i<bindVertexInputSets.size (); ++i )
            {
                ...
                if ( geometryId == 0 )
                {
                    std::cerr << "No geometry for the current source id!" << endl;
                    return;
                }               
    ...
    void MaterialImporter::connectGeometries ()
    {
        ...
        while ( geometryBindingIter != mGeometryMaterialBindingsMap.end () )
        {
            ...
            {
                std::cerr << "No shading engine for the current geometry!" << endl;
                ++geometryBindingIter;
                continue;
            }
            ...
            for ( size_t transformInstance=0; transformInstance<numTransformInstances; ++transformInstance )
            {
                ...
                for ( size_t i=0; i<numPrimitiveElements; ++i )
                {
                    ...
                    {
                        std::cerr << "No material info for the current shading engine!" << endl;
                        continue;
                    }
                    const COLLADAFW::UniqueId& materialId = materialBinding->getReferencedMaterial ();
    ...
    void MaterialImporter::connectUVChooser ()
    {
        ...
        while ( it != mBindingTexCoordInputSetsMap.end () )
        {
            ...
            for ( size_t i=0; i<bindVertexInputSets.size (); ++i )
            {
                ...
                {
                    std::cerr << "No input element for the current source and primitive element!" << endl;
                    continue;
                }
