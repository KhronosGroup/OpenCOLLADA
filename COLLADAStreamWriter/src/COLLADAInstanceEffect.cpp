...
namespace COLLADA
{
    ...
    {
        sw->openElement( CSWC::COLLADA_ELEMENT_TECHNIQUE_HINT );

        if ( !mPlatform.empty() )
            sw->appendAttribute ( CSWC::COLLADA_ATTRIBUTE_PLATFORM, mPlatform );

        sw->appendAttribute ( CSWC::COLLADA_ATTRIBUTE_REF, mRefNC );

        if ( !mProfileNC.empty() )
            sw->appendAttribute ( CSWC::COLLADA_ATTRIBUTE_PROFILE, mProfileNC );
    ...
    {
        mTagCloser = mSW->openElement ( CSWC::COLLADA_ELEMENT_INSTANCE_EFFECT );

        if ( !mSid.empty() )
            mSW->appendAttribute ( CSWC::COLLADA_ATTRIBUTE_SID, mSid );

        if ( !mName.empty() )
            mSW->appendAttribute ( CSWC::COLLADA_ATTRIBUTE_NAME, mName );

        mSW->appendURIAttribute ( CSWC::COLLADA_ATTRIBUTE_URL, mTargetURI );
    }
