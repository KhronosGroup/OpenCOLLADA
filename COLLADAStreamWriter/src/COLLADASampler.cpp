...
namespace COLLADA
{
    ...
    void Sampler::add ( StreamWriter* sw ) const
    {
        ...
        if ( mSamplerType != Sampler::SAMPLER_TYPE_STATE )
            sw->appendTextElement ( CSWC::COLLADA_ELEMENT_SOURCE, mSource );

        if ( mWrap_s != Sampler::WRAP_MODE_WRAP && mWrap_s != Sampler::WRAP_MODE_UNSPECIFIED )
            sw->appendTextElement ( CSWC::COLLADA_ELEMENT_WRAP_S, getWrapModeString ( mWrap_s ) );

        if ( mWrap_t != Sampler::WRAP_MODE_WRAP && mWrap_t != Sampler::WRAP_MODE_UNSPECIFIED )
            sw->appendTextElement ( CSWC::COLLADA_ELEMENT_WRAP_T, getWrapModeString ( mWrap_t ) );

        if ( mWrap_p != Sampler::WRAP_MODE_WRAP && mWrap_p != Sampler::WRAP_MODE_UNSPECIFIED )
            sw->appendTextElement ( CSWC::COLLADA_ELEMENT_WRAP_P, getWrapModeString ( mWrap_p ) );

        if ( mMinFilter != Sampler::SAMPLER_FILTER_NONE )
            sw->appendTextElement ( CSWC::COLLADA_ELEMENT_MINFILTER, getSamplerFilterString ( mMinFilter ) );

        if ( mMagFilter != Sampler::SAMPLER_FILTER_NONE )
            sw->appendTextElement ( CSWC::COLLADA_ELEMENT_MAGFILTER, getSamplerFilterString ( mMagFilter ) );

        if ( mMipFilter != Sampler::SAMPLER_FILTER_NONE )
            sw->appendTextElement ( CSWC::COLLADA_ELEMENT_MIPFILTER, getSamplerFilterString ( mMipFilter ) );

        if ( mUseBorderColor )
        {
            sw->openElement ( CSWC::COLLADA_ELEMENT_BORDER_COLOR );
        ...
        if ( writeMipMapLevel )
        {
            sw->openElement ( CSWC::COLLADA_ELEMENT_MIPMAP_MAXLEVEL );
        ...
        if ( mMipmapBias != 0.0f )
        {
            sw->openElement ( CSWC::COLLADA_ELEMENT_MIPMAP_BIAS );
