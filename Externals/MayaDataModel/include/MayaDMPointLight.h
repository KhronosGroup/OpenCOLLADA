...
public:
	...
	void setFogRadius(float fr){if(fr == 1.0) return; fprintf_s(mFile, "setAttr \".fr\" %f;\n", fr);}
	/*The radius of the sphere that contains the fog.*/
	void setFogRadius(const FloatID& fr){fprintf_s(mFile,"connectAttr \"");fr.write(mFile);fprintf_s(mFile,"\" \"%s.fr\";\n",mName.c_str());}
	...
	/*The type of the fog.*/
	void setFogType(unsigned int ft){if(ft == 0) return; fprintf_s(mFile, "setAttr \".ft\" %i;\n", ft);}
	/*The type of the fog.*/
	void setFogType(const UnsignedintID& ft){fprintf_s(mFile,"connectAttr \"");ft.write(mFile);fprintf_s(mFile,"\" \"%s.ft\";\n",mName.c_str());}
	/*
	...
	*/
	...
	void setFarPointWorldX(float fwx){if(fwx == 0.0) return; fprintf_s(mFile, "setAttr \".fw.fwx\" %f;\n", fwx);}
	/*The y-component of the world far-position.*/
	void setFarPointWorldY(float fwy){if(fwy == 0.0) return; fprintf_s(mFile, "setAttr \".fw.fwy\" %f;\n", fwy);}
	/*The z-component of the world far-position.*/
	void setFarPointWorldZ(float fwz){if(fwz == 0.0) return; fprintf_s(mFile, "setAttr \".fw.fwz\" %f;\n", fwz);}
	/*The fog intensity.*/
	...
	*/
	...
	void setMiLightProfile(const GenerictypeddataID& mlp){fprintf_s(mFile,"connectAttr \"");mlp.write(mFile);fprintf_s(mFile,"\" \"%s.mrc.mlp\";\n",mName.c_str());}
	/*Connect this to the fog geometry.*/
	MessageID getFogGeometry(){char buffer[4096];sprintf_s (buffer, "%s.fg",mName.c_str());return (const char*)buffer;}
	/*The radius of the sphere that contains the fog.*/
	FloatID getFogRadius(){char buffer[4096];sprintf_s (buffer, "%s.fr",mName.c_str());return (const char*)buffer;}
	/*Connect this to the light glow.*/
	MessageID getLightGlow(){char buffer[4096];sprintf_s (buffer, "%s.lg",mName.c_str());return (const char*)buffer;}
	/*The type of the fog.*/
	UnsignedintID getFogType(){char buffer[4096];sprintf_s (buffer, "%s.ft",mName.c_str());return (const char*)buffer;}
	/*The fog intensity.*/
	...
	*/
	...
	/*Port to connect custom mental ray node of type light profile.*/
	GenerictypeddataID getMiLightProfile(){char buffer[4096];sprintf_s (buffer, "%s.mrc.mlp",mName.c_str());return (const char*)buffer;}
protected:
