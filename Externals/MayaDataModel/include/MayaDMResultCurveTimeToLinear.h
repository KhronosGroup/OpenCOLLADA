...
*/
#ifndef __MayaDM_RESULTCURVETIMETOLINEAR_H__
#define __MayaDM_RESULTCURVETIMETOLINEAR_H__
...
This node is the result animation curve created for situations wherein
        the attribute expects "distance" (linear) data and is driven by "time".<p/>

        For more information, see the documentation of the Parent "resultCurve"
        node.<p/>
*/
class ResultCurveTimeToLinear : public ResultCurve
{
public:
...
public:
	ResultCurveTimeToLinear(FILE* file,const std::string& name,const std::string& parent=""):ResultCurve(file, name, parent, "resultCurveTimeToLinear"){}
	...
	/*
	Evaluated distance value of the result animation curve at the input
	time
	*/
...
protected:
	ResultCurveTimeToLinear(FILE* file,const std::string& name,const std::string& parent,const std::string& nodeType):ResultCurve(file, name, parent, nodeType) {}
private:

};
}//namespace MayaDM
#endif//__MayaDM_RESULTCURVETIMETOLINEAR_H__
