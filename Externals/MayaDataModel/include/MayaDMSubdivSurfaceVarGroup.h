...
This node will automatically create the transform/nurbsSurface pairs below
 ...
 transform/subdivSurface pairs are created.  If the number decreases, the
...
public:
	...
	/*The input surfaces*/
	void setCreate(size_t cr_i,const SubdivisionSurfaceID& cr){fprintf_s(mFile,"connectAttr \"");cr.write(mFile);fprintf_s(mFile,"\" \"%s.cr[%i]\";\n",mName.c_str(),cr_i);}
	/*The output surfaces*/
	void setLocal(size_t l_i,const SubdivisionSurfaceID& l){fprintf_s(mFile,"connectAttr \"");l.write(mFile);fprintf_s(mFile,"\" \"%s.l[%i]\";\n",mName.c_str(),l_i);}
	/*The input surfaces*/
	const SubdivisionSurfaceID& getCreate(size_t cr_i){char buffer[4096];sprintf_s (buffer, "%s.cr[%i]",mName.c_str(),cr_i);return (const char*)buffer;}
	/*The output surfaces*/
	const SubdivisionSurfaceID& getLocal(size_t l_i){char buffer[4096];sprintf_s (buffer, "%s.l[%i]",mName.c_str(),l_i);return (const char*)buffer;}
protected:
