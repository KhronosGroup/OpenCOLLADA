...
namespace DAE23ds
{
	...
	void Writer::_writeGeometry_lib3ds(const COLLADAFW::Geometry* geometry)
	{
			...
			lib3ds_file_insert_mesh(_3dsfile, _3dsmesh, -1);
			{
				...
				{
					//position
					if(COLLADAFW::DFI::DATA_TYPE_FLOAT==positions.getType())
					{
					...
					}else if(COLLADAFW::DFI::DATA_TYPE_DOUBLE==positions.getType()){
					...
					if(COLLADAFW::DFI::DATA_TYPE_FLOAT==uvs.getType())
					{
					...
					}else if(COLLADAFW::DFI::DATA_TYPE_DOUBLE==uvs.getType()){
