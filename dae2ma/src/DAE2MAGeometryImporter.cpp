...
//     }
                    ...
                    for ( size_t groupedVtxIndex=0; groupedVtxIndex<groupedVtxCount; ++groupedVtxIndex )
                    {
                        ...
                        for ( size_t j=2; j<indexCount; ++j )
                        {
                            ...
                            for ( size_t n=0; n<3; ++n, ++index )
                            {
                                ...
                                const COLLADAFW::DFI::DataType type = normals.getType ();
                                switch ( type )
                                {
        ...
        MayaDM::Mesh &meshNode )
    {
        ...
        for ( size_t i=0, index=0; i<count; i+=stride, ++index )
        {
            ...
            const COLLADAFW::DFI::DataType type = positions.getType ();
            switch ( type )
            {
            case COLLADAFW::DFI::DATA_TYPE_FLOAT:
                {
...
//                     std::cerr << "Just 2d uv set data will be imported! " << std::endl;
                ...
                for ( size_t j=0; j<indicesCount; j+=stride, ++index )
                {
                    ...
                    // Write the values 
                    const COLLADAFW::DFI::DataType type = uvCoords.getType ();
                    switch ( type )
                    {
                    case COLLADAFW::DFI::DATA_TYPE_FLOAT:
                        {
...
//        meshNode.setColorSetSize ( sumUVSetPoints );
        ...
        for ( size_t c=0; c<numColorSets; ++c )
        {
            ...
            if ( originalMesh )
            {
                ...
                for ( size_t i=0; i<indicesCount; i+=stride, ++index )
                {
                    ...
                    {
                        // Write the values 
                        const COLLADAFW::DFI::DataType type = colors.getType ();
                        switch ( type )
                        {
                        case COLLADAFW::DFI::DATA_TYPE_FLOAT:
                            {
        ...
        const size_t vertexIndex )
    {
        ...
        double dx, dy, dz; 
        if ( meshPositions.getType () == COLLADAFW::DFI::DATA_TYPE_FLOAT )
        {
        ...
        else if ( meshPositions.getType () == COLLADAFW::DFI::DATA_TYPE_DOUBLE )
        {
