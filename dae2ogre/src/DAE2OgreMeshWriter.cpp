...
namespace DAE2Ogre
{
	...
	void MeshWriter::addTupleIndex( const Tuple& tuple )
	{
		...
		if ( it == mTupleMap.end() )
		{
			...
			if ( mMeshPositions.getType() == COLLADAFW::DoubleFloatOrIntArray::DATA_TYPE_DOUBLE )
			{
			...
			if ( mHasNormals )
			{
				if ( mMeshNormals.getType() == COLLADAFW::DoubleFloatOrIntArray::DATA_TYPE_DOUBLE )
				{
			...
			if ( mHasUVCoords )
			{
				if ( mMeshUVCoordinates.getType() == COLLADAFW::DoubleFloatOrIntArray::DATA_TYPE_DOUBLE )
				{
